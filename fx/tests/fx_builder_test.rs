...
// This case is actually the default derived macro's behavior
#[cfg(test)]
mod multiple_schemed_table_builder {
    ...
    #[derive(Debug)]
    struct UsersCSBuild<T: Eclectic> {
        result: FxBatches<T>,
        buffer: Option<UsersEBuild<T>>,
        _e: PhantomData<E>,
    }

    impl FxCollectionBuilder<true, UsersEBuild<ChunkArr>, Users, FxTable<3>, usize, ChunkArr>
        for UsersCSBuild<ChunkArr>
    {
        fn new() -> FxResult<Self> {
            let schema = NullableOptions::indexed_true([2]).gen_schema(
                ["id", "name", "check"],
            ...
            let result = FxTable::<3>::empty_with_schema(schema);
            let buffer = Some(Users::gen_arraa_builder());
        }

        fn mut_buffer(&mut self) -> Option<&mut Users> {
            todo!()
        }

        fn set_buffer(&mut self, buffer: Users) {
            todo!()
        }

        fn take_buffer(&mut self) -> Option<Users> {
            todo!()
        }

        fn mut_result(&mut self) -> &mut usize {
            todo!()
        }

        fn take_result(self) -> usize {
            todo!()
        }
    }
}
